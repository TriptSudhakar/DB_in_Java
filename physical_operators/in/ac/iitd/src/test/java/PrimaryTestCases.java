...
//            String query="Select rental_id, rent_date, amount from \n" +
//                    "(SELECT rental.rental_id as rental_id, rental.rental_date as rent_date, payment.amount as amount \n" +
//                    "FROM rental left JOIN payment ON rental.rental_id  = payment.rental_id offset 14595) where amount < 100";
//            String query = "select rental.rental_id, rental.rental_date, payment.amount\n" +
//                    "from rental, payment\n" +
//                    "where rental.rental_id = payment.rental_id";
//            String query = "select city_id / 2.0 + 1 as city_id from city where city_id < 160";
//            String query = "select city_id from city group by city_id having max(country_id) < 100";
//            String query = "select first_name from actor where first_name < 'A'";
//            String query="Select rental_id, rent_date, amount from \n" +
//                    "(SELECT rental.rental_id as rental_id, rental.rental_date as rent_date, payment.amount as amount \n" +
//                    "FROM rental LEFT JOIN payment ON rental.rental_id  = payment.rental_id offset 14595) order by amount";
//            String query = "select first_name, count(*) from actor group by first_name having count(*) > 1";
//            String query = "select first_name,last_name from actor";
//            String query = "select \"language\".name, count(film.film_id) \n" +
//                    "from \"language\" left join film on film.language_id = \"language\".language_id \n" +
//                    "group by \"language\".name";
//            String query = "with avg_film_actor as (\n" +
...
//                    "    ) as a\n" +
//                    ")\n" +
//                    "select actor_id, count(film_id), average\n" +
//                    "from film_actor \n" +
//                    "cross join avg_film_actor\n" +
//                    "group by actor_id, average\n" +
//                    "limit 10";
            String query = "select (actor_id + 2)/3.0 from actor";
            RelNode relNode = createRelNode(query, calciteConnection);
            List<Object []> result = eval(relNode, calciteConnection);
